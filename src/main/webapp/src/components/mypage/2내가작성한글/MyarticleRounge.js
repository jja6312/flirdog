...
const MyarticleRounge = () => {
  ...
  useEffect(() => {
    ...
    // 리스트 전체조회
    axios
      .get(`/mypage/uploadListBoard?id=${userId}`)
      .then((res) => {
      ...
      <Container className="px-10 mt-7">
        ...
        <Row className={`${Mypage.Myarticle2}`}>
          ...
          <Col xs={6} md={4} className={Mypage.Imagecenter}>
            <div className={Mypage.Myarticle2}>등록한 글이 없습니다.</div>
          </Col>
          <Col xs={3} md={4}></Col>
        </Row>
        {/* 내용 출력 */}
          ...
          <Col xs={6} md={4} className={Mypage.Imagecenter}>
            {list.map((item) => (
              ...
              <Link className={Mypage.subjectA} to={`/mypage/FreeBoardOne`}>
                <div
                  key={item.id}
                  className={Mypage.MydogProfileMarging}
                  style={{
                    marginBottom: "10px",
