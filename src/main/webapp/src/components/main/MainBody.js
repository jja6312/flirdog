...
const MainBody = () => {
  ...
  useEffect(() => {
    ...
    if (userStored) {
      ...
      const userIdValue = userParsed.id || userParsed.user?.id;
      if (userIdValue) {
  ...
  return (
    <>
      {userId && <Chatting isOpenChatting={isOpenChatting} setIsOpenChatting={setIsOpenChatting} userId={userId} isCreated={isCreated} />}
