...
const DateList = () => {
    ...
    // 전체 목록 조회
    useEffect(() => {
        axios.get('http://localhost:8080/date/getAllMatchingList')
          .then((res) => {
            console.log("주소")
    ...
    // 상위 3개 조회
    useEffect(() => {
    axios.get('http://localhost:8080/date/getTopMatchingThree')
        .then((res) => {
        console.log("주소")
        console.log(res.data);
    ...
    const handlePurposeCheckboxChange = (purpose) => {
        setSelectedPurpose((prevPurpose) => (prevPurpose === purpose ? null : purpose));
    };
    const handleNeutralizationCheckboxChange = (neutralization) => {
        setSelectedNeutralization((prevNeutralization) => (prevNeutralization === neutralization ? null : neutralization));
    };
    const handleMatchingStateCheckboxChange = (matchingState) => {
        setSelectedMatchingState((prevMatchingState) => (prevMatchingState === matchingState ? null : matchingState));
    };
                        ...
                        <div key={index} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                            ...
                            <div className='d-flex justify-content-center align-items-center'>
                                <div className='justify-content-center align-items-center'
                                    ...
                                    <img
                                        src={`${
                                            matchingItem.image === "/image/nullImage/nullImage1.png"
                                                ? matchingItem.image
                                                : `https://kr.object.ncloudstorage.com/bitcamp-edu-bucket-95/${matchingItem.image.split(',')[0]}`
                                            }`}
                                        alt=""
                                    ...
                                    <div style={{display:'flex',
                                        ...
                                        >
                                        <div style={{ fontWeight: 'bold', height: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                            {[1, 2, 3, 4, 5].map((starIndex) => {
                                                ...
                                                return (
                                                    ...
                                                    width={40}
                                                    alt="별"
                                                    style={{ marginRight: starIndex === 5 ? '0' : '40px'}}
                                                    />
                                                );
                                                })}
                                        </div>
                                ...
                                <div className={dateList.filterDateContent} style={{ backgroundColor: 'white', 
                                    padding:'2%',
                                    margin:'3%',
                                    height:'100%',
                                    marginRight:'5%',
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', height: '600px', 
                                        ...
                                        <img alt='' src='/image/main/likeBone.png' width={50} />
                                        &nbsp;&nbsp;<span style={{fontSize:'2vw'}}>{matchingItem.communityScore}</span>
                                        </div>
                                    </div>
                                </div>

                                <div>
                                    <div style={{ position: 'absolute', top: 5, left: 65, width: '30%'}}>
                                        <div style={{ display: 'flex', 
            ...
            <Container className='px-10'>
                ...
                <div className={dateList.filterList}>
                                ...
                                <div className={dateList.filterContent}>
                                    ...
                                    <div className='neutralizationCheck' style={{
                                        ...
                                        <div className={`${dateCheck.neutralizationCheckBox} d-flex justify-content-left`}>
                                            ...
                                            onChange={() => handleNeutralizationCheckboxChange('중성화')}
                                            />
                                    ...
                                    <div className='filterAddress' style={{
                                        ...
                                        <Dropdown>
                                            ...
                                            style={{ maxHeight: '200px', overflowY: 'auto' }}
                                                >
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-1" onClick={() => handleRegionSelect('전체선택')}>지역전체</Dropdown.Item>   
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-1" onClick={() => handleRegionSelect('서울')}>서울</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-2" onClick={() => handleRegionSelect('부산')}>부산</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-3" onClick={() => handleRegionSelect('대구')}>대구광역시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-4" onClick={() => handleRegionSelect('인천')}>인천광역시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-5" onClick={() => handleRegionSelect('광주')}>광주광역시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-6" onClick={() => handleRegionSelect('대전')}>대전광역시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-7" onClick={() => handleRegionSelect('울산')}>울산광역시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-8" onClick={() => handleRegionSelect('세종')}>세종특별시</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-9" onClick={() => handleRegionSelect('경기')}>경기</Dropdown.Item>
                                                ...
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-16" onClick={() => handleRegionSelect('강원')}>강원</Dropdown.Item>
                                                <Dropdown.Item style={{ opacity: 1 }} href="#/action-17" onClick={() => handleRegionSelect('제주')}>제주</Dropdown.Item>
                                            </Dropdown.Menu>
                                        </Dropdown>
                                        </div>
                                    </div>
                    ...
                    <div className={dateList.filterDateList}>
                        ...
                        {allMatchingList
                        .filter(matchingItem =>
                            (!selectedGender || matchingItem.dogGender === selectedGender) &&
                            (!selectedPurpose || matchingItem.matchingPurpose === selectedPurpose) &&
                            (!selectedNeutralization || matchingItem.neutralization === selectedNeutralization) &&
                            (!selectedMatchingState || matchingItem.matchingState === selectedMatchingState) &&
                            (selectedRegion === '지역 선택' || selectedRegion === '전체선택' || (matchingItem.matchingAddress && matchingItem.matchingAddress.includes(selectedRegion)))
                        )
                        .map((matchingItem, index) => (
                            //데이터가 존재하는지 확인 후 렌더링
                            //상세보기 링크
                            matchingItem.id && (
                                <Link
                                    ...
                                    key={index}
                                >
                            <div key={index} className={dateList.filterDate}
                                ...
                                <div className={dateList.filterDateImg}>
                                    ...
                                    <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '10px', width: '100%', height: 200, overflow: 'hidden' }}>
                                        <img
                                            alt=''
                                            src={`${
                                            matchingItem.image === "/image/nullImage/nullImage1.png"
                                        ...
                                        /> 
                                    <div style={{ position: 'absolute', top: 10, left: 10, width: '30%'}}>
                                        <div style={{ display: 'flex', 
                                            flexDirection: 'column', 
                                            justifyContent: 'center',
                                            alignItems: 'center', // 수평 가운데 정렬을 위한 속성
