import React from 'react';
import Container from 'react-bootstrap/esm/Container';
import { Link } from 'react-router-dom';

const SomoimMainList = () => {
    return (
        <>
            <Container className="px-8 col-md-12 col-sm-8 col-4">
                <div className='row'>
                    {/* <div className="mt-2 mb-2 d-flex justify-content-center align-items-center" style={{border: '1px solid red'}}> */}
                    <div className="mt-2 mb-2 d-flex justify-content-center align-items-center">
                        {/* <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', border: '1px solid red', padding: 0}}> */}
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    <Link to='/somoim/detailMain'>우리 동네 강아지 자랑해요 - 반려견 친목모임</Link>
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                    </div>
                </div>
                <div className='row'>
                    <div className="mt-2 mb-2 d-flex justify-content-center align-items-center">
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    <Link to='/somoim/detailMain'>우리 동네 강아지 자랑해요 - 반려견 친목모임</Link>
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                    </div>
                </div>
                <div className='row'>
                    <div className="mt-2 mb-2 d-flex justify-content-center align-items-center">
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    <Link to='/somoim/detailMain'>우리 동네 강아지 자랑해요 - 반려견 친목모임</Link>
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                        <div className='d-flex justify-content-center' style={{ width:'344px',height: '331px', flex: 1, position: 'relative', background: 'white', borderRadius: 3, overflow: 'hidden', padding: 0}}>
                            <img className='img-thumbnail' style={{width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', borderRadius: 13}} src='/image/somoim/highlight1.png' alt='somoimHighlight' />
                            <div style={{top: 48, position: 'absolute', color: 'white', fontSize: 13, fontFamily: 'Ribeye', fontWeight: '400', wordWrap: 'break-word'}}>
                                    우리 동네 강아지 자랑해요 - 반려견 친목모임
                            </div>
                        </div>
                    </div>
                </div>
            </Container>
        </>
    );
};

export default SomoimMainList;